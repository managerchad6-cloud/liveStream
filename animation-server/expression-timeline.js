...
function buildExpressionPlan({ message, character, listener, durationSec, limits }) {
  ...
  function addBrowAction(t, emote, amount, durationMs) {
    if (t - lastBrowTime < MIN_BROW_GAP_MS) return;
    plan.actions.push({
      t: Math.round(t),
      type: 'brow',
      target: character,
      emote,
      amount,
      durationMs: Math.max(1400, Math.round(durationMs))
    });
    lastBrowTime = t;
  }
  ...
  for (let i = 0; i < sentenceTimings.length; i++) {
    ...
    if (sent.startMs - lastEyeTime >= MIN_EYE_GAP_MS) {
      plan.actions.push({
        t: sent.startMs,
        type: 'eye',
        target: character,
    ...
    if (sent.durationMs > 1200) {
      ...
      if (glance1Time - lastEyeTime >= MIN_EYE_GAP_MS) {
        plan.actions.push({
          t: glance1Time,
          type: 'eye',
          target: character,
        ...
        lastEyeTime = glance1Time;
      }

      // Back to listener
      {
        const backTime = glance1Time + Math.round(randRange(rng, 320, 460));
        if (backTime - lastEyeTime >= MIN_EYE_GAP_MS) {
          plan.actions.push({
            t: backTime,
            type: 'eye',
            target: character,
      ...
      if (sent.durationMs > 2500) {
        ...
        if (glance2Time - lastEyeTime >= MIN_EYE_GAP_MS) {
          plan.actions.push({
            t: glance2Time,
            type: 'eye',
            target: character,
          ...
          lastEyeTime = glance2Time;
        }

        {
          const back2Time = glance2Time + Math.round(randRange(rng, 300, 420));
          if (back2Time - lastEyeTime >= MIN_EYE_GAP_MS) {
            plan.actions.push({
              t: back2Time,
              type: 'eye',
              target: character,
    ...
    if (hasPause || sent.durationMs > 1600) {
      ...
      if (thinkTime - lastEyeTime >= MIN_EYE_GAP_MS) {
        plan.actions.push({
        ...
        lastEyeTime = thinkTime;
      }

      const backThinkTime = thinkTime + randRange(rng, 260, 420);
      if (backThinkTime - lastEyeTime >= MIN_EYE_GAP_MS) {
        plan.actions.push({
          t: Math.round(backThinkTime),
          type: 'eye',
          target: character,
          look: 'listener',
          amount: randRange(rng, 0.4, 0.5),
  ...
  if (browCount < 10) {
    ...
    for (let i = 0; i < browAdds; i++) {
      ...
      actions.push({
        ...
        durationMs: Math.round(randRange(rng, 1400, 2200))
      });
    }
  }
